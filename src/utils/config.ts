...
const DEFAULT_SETTINGS: Settings = {
  ...
  llm: {
    ...
    maxTokens: 1000,
    temperature: 0,
    systemPrompt: "你是一个专业的广告数据截图识别助手。请仔细分析图片中的广告数据表格。\n【极度重要：必须彻底忽略并跳过带有“总计”、“合计”字样的行！如果某一行没有明确的带有APP/APK字样的广告系列名称，绝对不能作为数据提取，直接丢弃！】\n\n对于每一行有效数据，提取以下字段：\n- 渠道名：在“广告系列名称”列中找出“APK”或“APP”（不区分大小写），找到与它对应的“纯数字”（这串数字可能紧挨着，也可能隔着“-”或“-包”等杂字，例如“-包85”）。截取规则：保留从开头一直到“APK”或“APP”的部分，然后**直接拼接上那串数字**。必须把中间的“-包”或“-”等杂字去掉，并且严格丢弃这串数字之后的所有字母、符号及后缀（例如“-TT”、“-GG”等）。（例如：42122-APP001-GG-Jigsaw 提取为 42122-APP001；22666-1351-APK-包85-TT-MOON 提取为 22666-1351-APK85）。\n- 消耗/U：“费用”列中找到费用数值，去掉货币符号和千分位逗号，保留纯数字和小数点（如 $1,234.56 提取为 1234.56）。\n- 展示：“展示次数”，去掉千分位逗号（如 1,234 提取为 1234）。\n- 点击量：“点击次数”，去掉千分位逗号，结果放在“点击量”。\n\n请必须返回一个 JSON 对象，其中包含一个 \"data\" 键，其值为包含上述字段的数组。\n格式示例：\n{\n  \"data\": [\n    {\"渠道名\": \"42122-APP001\", \"消耗/U\": 1234.56, \"展示\": 1234, \"点击量\": 100}\n  ]\n}\n如果图片不包含有效数据或全是总计行，请返回：{ \"data\": [] }"
  },
  excel: {
