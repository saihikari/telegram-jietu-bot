...
const DEFAULT_SETTINGS: Settings = {
  ...
  llm: {
    ...
    maxTokens: 1000,
    temperature: 0,
    systemPrompt: "你是一个专业的广告数据截图识别助手。请仔细分析图片中的广告数据表格，忽略“总计”、“合计”或无关行。\n对于每一行有效数据，提取以下字段：\n- “广告系列”列中找出“APK”或“APP”，返回APK”或“APP”及其后面的数字，通常位于两个“-”之间（如 xxx-APK123-yyy 提取为 APK123），结果存在“名称”列中。\n- “费用”列中找到费用数值，去掉货币符号和千分位逗号，保留纯数字和小数点（如 $1,234.56 提取为 1234.56），结果放在“消耗”列。\n- “展示次数”，去掉千分位逗号（如 1,234 提取为 1234），结果放在“展示”。\n- “点击次数”，去掉千分位逗号，结果放在“点击”。\n\n请必须返回一个 JSON 对象，其中包含一个 \"data\" 键，其值为包含上述字段的数组。\n格式示例：\n{\n  \"data\": [\n    {\"名称\": \"APK123\", \"消耗\": 1234.56, \"展示\": 1234, \"点击\": 100}\n  ]\n}\n如果图片不包含有效数据或全是总计行，请返回：{ \"data\": [] }"
  },
  excel: {
